...
<head>
    ...
    <title>Laporan Daftar Makanan & Minuman Kucing</title>
    <link rel="stylesheet" href="css/print.css"
</head>
<body>
    <h1 style="text-align: center;"> DAFTAR MAKANAN DAN MINUMAN KUCING</h1>
...
while ($row = mysqli_fetch_assoc($result)) {
    $html .= '<tr>
            <td>' . $i . '</td>
            <td> <img src="./assets/imgFood/' . $row['gambar'] . '" height="100" width="100"/> </td>
            <td>' . $row["nama"] . '</td>
            <td>' . $row["harga"] . '</td>
            <td>' . $row["stok"] . '</td>
            <td>' . $row["deskripsi"] . '</td>
...
$mpdf->Output('Laporan Daftar Makanan & Minuman Kucing.pdf', 'I');
